...
{
    public TargobankPDFExtractor(Client client) throws IOException
    {
        super(client);
    ...
    {
        DocumentType type = new DocumentType("Effektenabrechnung");
        ...
        getHeaderSectionsTransaction(pdfTransaction)
                .wrap(BuySellEntryItem::new);
//        pdfTransaction.section("wkn", "isin", "name", "currency") //
//                        .match("^(?<name>.*) WKN (?<wkn>[^ ]{6})$") //
//                        .match("(.*[ ])?ISIN (?<isin>[^ ]{12})$") //
//                        .find("Nominal\\/St.ck ST.*")
//                        .match("^Kurs (?<currency>\\w{3}+) .*$")
//                        .assign((t, v) -> t.setSecurity(getOrCreateSecurity(v)))
//
//                        .section("shares") //
//                        .match("^Nominal\\/St.ck ST (?<shares>[\\d.]+(,\\d+)?) .*") //
...
//                        .match("Gesamtbetrag (?<amount>[\\d.]+,\\d+) - (?<currency>\\w{3}+)") //
//                        .assign((t, v) -> {
...
//                        .wrap(BuySellEntryItem::new);
    ...
    private void addPreemptiveBuyTransaction()
    {
        ...
        getHeaderSectionsTransaction(pdfTransaction).wrap(BuySellEntryItem::new);
    ...
    @SuppressWarnings("nls")
    private Transaction<BuySellEntry> getHeaderSectionsTransaction(Transaction<BuySellEntry> pdfTransaction)
    {
      return pdfTransaction.section("wkn", "isin", "name", "currency") //
                      .match("^(?<name>.*) WKN (?<wkn>[^ ]{6})$") //
                      .match("(.*[ ])?ISIN (?<isin>[^ ]{12})$") //
                      .find("Nominal[ ]*\\/[ ]*St.ck ST.*")
                      .match("^Kurs (?<currency>\\w{3}+) .*$")
    ...
    private void addFeesSectionsTransaction(Transaction<BuySellEntry> pdfTransaction)
    {
        pdfTransaction.section("currency", "stockfees").optional()
                        .match("(^.*)(B.rsenplatzentgelt) (?<stockfees>\\d{1,3}(\\.\\d{3})*(,\\d{2})?) - (?<currency>\\w{3}+)")
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("stockfees"))))))

                        .section("currency", "brokerage").optional()
                        .match("(^.*)(Provision) (?<brokerage>\\d{1,3}(\\.\\d{3})*(,\\d{2})?) - (?<currency>\\w{3}+)")
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("brokerage"))))))

                        .section("currency", "fee").optional()
                        .match("(^.*)(Handelsentgelt) B.rse (?<fee>\\d{1,3}(\\.\\d{3})*(,\\d{2})?) - (?<currency>\\w{3}+)")
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("fee"))))))

                        .section("currency", "fee").optional()
                        .match("(^.*)(Transaktionsentgelt) B.rse (?<fee>\\d{1,3}(\\.\\d{3})*(,\\d{2})?) - (?<currency>\\w{3}+)")
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("fee"))))))

                        .section("currency", "courtage").optional()
                        .match("(^.*)(Courtage) (?<courtage>\\d{1,3}(\\.\\d{3})*(,\\d{2})?) - (?<currency>\\w{3}+)")
                        .assign((t, v) -> t.getPortfolioTransaction().addUnit(new Unit(Unit.Type.FEE,
                                        Money.of(asCurrencyCode(v.get("currency")), asAmount(v.get("courtage"))))));
